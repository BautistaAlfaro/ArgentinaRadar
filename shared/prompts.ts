 ...
 *
 * Usage:
 *   import { buildCategoryPrompt, getCategoryConfig, NEWS_CATEGORIES } from '../../shared/prompts';
 *
...
export interface CategoryConfig {
  /** Telegram badge label */
  badge: string;
  /** Icon emoji */
  emoji: string;
  /** Mood description for the image generation prompt */
  mood: string;
  /** Primary scene type for this category */
  sceneStyle: string;
  ...
  composition: string;
  /** Category-specific color accent variation */
  colorAccent: string;
}
...
export const CATEGORY_CONFIGS: Record<NewsCategory, CategoryConfig> = {
  economia: {
    badge: '📈 ECONOMÍA',
    emoji: '💰',
...
/**
 * Clean the title for embedding in prompts — strip special characters
 * that could confuse the image generator.
 */
 ...
 *
 * The prompt follows the Argentina Radar design system:
 * - Bloomberg/Reuters premium journalism aesthetic
 * - Dark navy #07111F background
 * - Electric blue #00A3FF accents
 * - Category badge prominently displayed
 * - Scene tailored to category context
 *
 * @param category  News category key
 * @param title     Article headline
 * @param source    Source name (e.g. "Infobae", "Clarín")
 * @returns         Full prompt string ready for Gemini image generation
 */
...
export function getCategoryConfig(category: string): CategoryConfig {
  return CATEGORY_CONFIGS[category as NewsCategory] || CATEGORY_CONFIGS.general;
}
